...
const fetchBonuses = async (page = 1) => {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/api/admin/bonuses/binary?${params.toString()}`, {
      headers: {
    ...
    if (!response.ok) throw new Error('Failed to fetch binary bonuses')
    ...
    console.error('Error fetching binary bonuses:', err)
