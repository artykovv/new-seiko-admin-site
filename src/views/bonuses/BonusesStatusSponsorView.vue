    ...
    <div v-else class="d-flex flex-column flex-grow-1 overflow-hidden">
      <div class="flex-grow-1 overflow-auto">
        ...
        >
          ...
          <template #cell-status_amount="{ value }">
            {{ formatCurrency(value) }}
          </template>
          <template #cell-sponsor_amount="{ value }">
            {{ formatCurrency(value) }}
          </template>
...
const columns = [
  ...
  { key: 'status_amount', label: 'Статусный' },
  { key: 'sponsor_amount', label: 'Спонсорский' },
...
const fetchBonuses = async (page = 1) => {
  ...
  try {
    ...
    let url = `${BACKEND_API_URL}/api/admin/bonuses/summary/status-sponsor?page=${page}&page_size=${pageSize.value}`
...
const issueBonus = async (item) => {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_API_URL}/api/admin/bonuses/issue/status-sponsor`, {
      method: 'POST',
      headers: {
